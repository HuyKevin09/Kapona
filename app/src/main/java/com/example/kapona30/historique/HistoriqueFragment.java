...
/**
 * Classe permettant l'affichage des produits ayant été recherchés ou scannés.
...
public class HistoriqueFragment extends Fragment {
     ...
     * Adapter à utiliser
     */
    private AdapterHistorique adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        dataList = MainActivity.database.consoDao().getAll("historique");

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
        recyclerView.setLayoutManager(linearLayoutManager);
        adapter = new AdapterHistorique(getActivity(), dataList);
        recyclerView.setAdapter(adapter);

        btReset.setOnClickListener(v -> {
            MainActivity.database.consoDao().reset(dataList);
            dataList.clear();
            dataList.addAll(MainActivity.database.consoDao().getAll("historique"));
